...
const getProfile = async (req, res) => {
    try {
        const [rows] = await pool.query(
            'SELECT id, ten_dang_nhap, ho_ten, email, so_dien_thoai, dia_chi, avatar, gioi_tinh, ngay_sinh, vai_tro, trang_thai_xac_thuc, ly_do_tu_choi, mat_khau FROM nguoi_dung WHERE id = ?',
            [req.user.id]
        );
        ...
        delete user.mat_khau; // Don't send password hash to frontend
...
const updateProfile = async (req, res) => {
    try {
        const { ten_dang_nhap, ho_ten, so_dien_thoai, gioi_tinh, ngay_sinh } = req.body;
        ...
        // Kiểm tra ten_dang_nhap có bị trùng không nếu người dùng muốn đổi
        if (ten_dang_nhap && ten_dang_nhap !== req.user.ten_dang_nhap) {
            const [existing] = await pool.query('SELECT id FROM nguoi_dung WHERE ten_dang_nhap = ? AND id != ?', [ten_dang_nhap, req.user.id]);
        ...
        await pool.query(
            'UPDATE nguoi_dung SET ten_dang_nhap = ?, ho_ten = ?, so_dien_thoai = ?, gioi_tinh = ?, ngay_sinh = ?, avatar = ? WHERE id = ?',
            [ten_dang_nhap || req.user.ten_dang_nhap, ho_ten || req.user.ho_ten, so_dien_thoai || req.user.so_dien_thoai, gioi_tinh || req.user.gioi_tinh, ngay_sinh || null, avatar, req.user.id]
        );
