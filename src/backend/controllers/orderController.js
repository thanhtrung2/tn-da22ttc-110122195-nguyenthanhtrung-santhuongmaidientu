...
const createOrder = async (req, res) => {
    ...
    try {
        ...
        // [Mới] Dọn dẹp các đơn hàng online chưa thanh toán (bỏ ngang) trước đó của user để nhả tồn kho và voucher
        const [abandoned] = await connection.query(
            "SELECT id, vi_voucher_id FROM don_hang WHERE nguoi_mua_id = ? AND phuong_thuc_thanh_toan IN ('vnpay', 'payos') AND trang_thai_thanh_toan = 'pending' AND trang_thai != 'da_huy'", 
            [req.user.id]
        );
        for (let row of abandoned) {
            const [items] = await connection.query("SELECT * FROM chi_tiet_don_hang WHERE don_hang_id = ?", [row.id]);
            for (let item of items) {
                await connection.query("UPDATE san_pham SET so_luong_ton = so_luong_ton + ? WHERE id = ?", [item.so_luong, item.san_pham_id]);
                if (item.bien_the_id) {
                    await connection.query("UPDATE san_pham_bien_the SET so_luong_ton = so_luong_ton + ? WHERE id = ?", [item.so_luong, item.bien_the_id]);
                }
            }
            if (row.vi_voucher_id) {
                await connection.query("UPDATE vi_voucher SET trang_thai = 'chua_dung', ngay_dung = NULL WHERE id = ?", [row.vi_voucher_id]);
                await connection.query("UPDATE khuyen_mai km JOIN vi_voucher vv ON km.id = vv.khuyen_mai_id SET km.da_dung = GREATEST(0, km.da_dung - 1) WHERE vv.id = ?", [row.vi_voucher_id]);
            }
            await connection.query("UPDATE don_hang SET trang_thai = 'da_huy', ly_do_huy = 'Hệ thống tự động hủy đơn bỏ ngang' WHERE id = ?", [row.id]);
        }
        ...
        for (const [shopId, items] of Object.entries(shopGroups)) {
            ...
            // Create order with vi_voucher_id
            const [orderResult] = await connection.query(
                `INSERT INTO don_hang (nguoi_mua_id, gian_hang_id, tien_hang, phi_van_chuyen, thue_vat, phi_admin, tong_tien, dia_chi_giao, so_dien_thoai, phuong_thuc_thanh_toan, ma_giao_dich, ghi_chu, vi_voucher_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                [req.user.id, shopId, tienHang, phiVanChuyen, thueVat, phiAdmin, tongTien, dia_chi_giao, so_dien_thoai, phuong_thuc_thanh_toan || 'cod', maGiaoDich, ghi_chu, validVoucher ? validVoucher.id : null]
            );
            ...
            // Gửi thông báo cho người bán có đơn hàng mới (Chỉ gửi nếu là COD)
        ...
        // Clear cart (Chỉ xóa nếu là COD. Online sẽ giữ lại cho đến khi thanh toán thành công)
        if (phuong_thuc_thanh_toan === 'cod') {
        ...
        // Phát realtime notification tới các seller (sau khi commit để tránh gửi khi rollback, và chỉ cho COD)
        if (phuong_thuc_thanh_toan === 'cod') {
            const io = getIO();
            if (io) {
...
const getMyOrders = async (req, res) => {
    try {
        ...
        let sql = `SELECT dh.*, gh.ten_gian_hang,
            ...
            WHERE dh.nguoi_mua_id = ? AND NOT (dh.phuong_thuc_thanh_toan IN ('vnpay', 'payos') AND dh.trang_thai_thanh_toan = 'pending')`;
...
const cancelUnpaidOrder = async (req, res) => {
    ...
    try {
        ...
        const [orders] = await connection.query(
            "SELECT id, vi_voucher_id FROM don_hang WHERE id = ? AND nguoi_mua_id = ? AND phuong_thuc_thanh_toan IN ('vnpay', 'payos') AND trang_thai_thanh_toan = 'pending' AND trang_thai != 'da_huy'",
            [orderId, req.user.id]
        );

        if (orders.length === 0) {
            await connection.rollback();
            return res.status(404).json({ success: false, message: 'Đơn hàng không tồn tại hoặc không thể hủy' });
        }
        ...
        await connection.query("UPDATE don_hang SET trang_thai = 'da_huy', ly_do_huy = 'Khách hàng xác nhận hủy thanh toán' WHERE id = ?", [row.id]);
        
        await connection.commit();
        res.json({ success: true, message: 'Hủy thanh toán thành công' });
    } catch (error) {
        if (connection) await connection.rollback();
        console.error('cancelUnpaidOrder error:', error);
...
const retryPayment = async (req, res) => {
    try {
        ...
        const [orders] = await pool.query(
            "SELECT id, tong_tien, phuong_thuc_thanh_toan, ma_giao_dich FROM don_hang WHERE id = ? AND nguoi_mua_id = ? AND phuong_thuc_thanh_toan IN ('vnpay', 'payos') AND trang_thai_thanh_toan = 'pending' AND trang_thai != 'da_huy'",
            [orderId, req.user.id]
        );

        if (orders.length === 0) {
            return res.status(404).json({ success: false, message: 'Đơn hàng không tồn tại hoặc đã xử lý' });
        }

        const order = orders[0];
        
        // Cần mã giao dịch mới để tạo link mới
        const newMaGiaoDich = Number(String(Date.now()).slice(-5) + Math.floor(1000 + Math.random() * 9000));
        await pool.query("UPDATE don_hang SET ma_giao_dich = ? WHERE id = ?", [newMaGiaoDich, order.id]);
        ...
        if (order.phuong_thuc_thanh_toan === 'vnpay') {
            ...
            vnp_Params['vnp_TxnRef'] = newMaGiaoDich;
            vnp_Params['vnp_OrderInfo'] = 'Thanh toan don hang ' + newMaGiaoDich;
            vnp_Params['vnp_OrderType'] = 'other';
            vnp_Params['vnp_Amount'] = order.tong_tien * 100;
        ...
        } else if (order.phuong_thuc_thanh_toan === 'payos') {
            ...
            const cancelUrl = `${process.env.APP_URL || 'http://localhost:3000'}/api/payment/payos/cancel?orderCode=${newMaGiaoDich}`;
            const returnUrl = `${process.env.APP_URL || 'http://localhost:3000'}/api/payment/payos/return?orderCode=${newMaGiaoDich}`;
            
            const paymentData = {
                orderCode: newMaGiaoDich,
                amount: Math.round(order.tong_tien),
                description: `Thanh toan don ${newMaGiaoDich}`.substring(0, 25),
                cancelUrl,
                returnUrl
            };
        ...
        res.status(500).json({ success: false, message: 'Lỗi server khi tạo link thanh toán mới' });
    }
};
...
const getShopOrders = async (req, res) => {
    try {
        ...
        let sql = `SELECT dh.*, nd.ho_ten as ten_nguoi_mua, nd.so_dien_thoai as sdt_nguoi_mua,
            ...
            WHERE dh.gian_hang_id = ? AND NOT (dh.phuong_thuc_thanh_toan IN ('vnpay', 'payos') AND dh.trang_thai_thanh_toan = 'pending')`;
        const params = [shop[0].id];
...
const createOrderBuyNow = async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Thông báo người bán (Chỉ gửi nếu thanh toán COD, online thì đợi thanh toán xong mới gửi)
            if ((phuong_thuc_thanh_toan || 'cod') === 'cod') {
...
// Mua lại đơn hàng
const rebuyOrder = async (req, res) => {
    try {
        const orderId = req.params.id;
        const [orders] = await pool.query('SELECT id FROM don_hang WHERE id = ? AND nguoi_mua_id = ?', [orderId, req.user.id]);
        if (orders.length === 0) {
            return res.status(404).json({ success: false, message: 'Đơn hàng không tồn tại' });
        }
        
        const [items] = await pool.query('SELECT san_pham_id, bien_the_id, so_luong FROM chi_tiet_don_hang WHERE don_hang_id = ?', [orderId]);
        
        let addedCount = 0;
        let skipCount = 0;

        for (const item of items) {
            const [product] = await pool.query('SELECT so_luong_ton, trang_thai FROM san_pham WHERE id = ?', [item.san_pham_id]);
            if (product.length === 0 || product[0].trang_thai !== 'active') {
                skipCount++;
                continue;
            }
            
            let stock = product[0].so_luong_ton;
            if (item.bien_the_id) {
                const [variant] = await pool.query('SELECT so_luong_ton FROM san_pham_bien_the WHERE id = ?', [item.bien_the_id]);
                if (variant.length === 0) {
                    skipCount++;
                    continue;
                }
                stock = variant[0].so_luong_ton;
            }
            ...
            const qtyToAdd = Math.min(item.so_luong, stock);
            const query = item.bien_the_id ? 
                'SELECT id, so_luong FROM gio_hang WHERE nguoi_mua_id = ? AND san_pham_id = ? AND bien_the_id = ?' :
                'SELECT id, so_luong FROM gio_hang WHERE nguoi_mua_id = ? AND san_pham_id = ? AND bien_the_id IS NULL';
            const params = item.bien_the_id ? [req.user.id, item.san_pham_id, item.bien_the_id] : [req.user.id, item.san_pham_id];
            
            const [existing] = await pool.query(query, params);

            if (existing.length > 0) {
                const newQty = Math.min(existing[0].so_luong + qtyToAdd, stock);
                ...
                await pool.query(
                    ...
                    [req.user.id, item.san_pham_id, item.bien_the_id || null, qtyToAdd]
                );
            }
            addedCount++;
        }
        ...
        if (addedCount === 0) {
            return res.json({ success: false, message: 'Các sản phẩm trong đơn này đã hết hàng hoặc ngừng kinh doanh' });
        }
        
        let msg = `Đã thêm ${addedCount} sản phẩm vào giỏ hàng.`;
        if (skipCount > 0) msg += ` Có ${skipCount} sản phẩm bị bỏ qua do không còn hàng.`;
        
        res.json({ success: true, message: msg });
    } catch (error) {
        console.error('rebuyOrder error:', error);
        res.status(500).json({ success: false, message: 'Lỗi server' });
    }
};

module.exports = {
    createOrder, getMyOrders, getOrderById, cancelOrder, cancelUnpaidOrder, retryPayment,
