...
if (isCloudinaryConfigured) {
    ...
    uploadProduct = multer({ storage: productStorage, limits: { fileSize: 50 * 1024 * 1024 } });
    ...
    const fileFilter = (req, file, cb) => {
        ...
        if (ext && mime) {
            ...
            cb(new Error('Chỉ chấp nhận file ảnh (jpg, png, gif, webp) và video (mp4, webm)'), false);
        }
    };

    uploadProduct = multer({ storage: createLocalStorage('products'), limits: { fileSize: 50 * 1024 * 1024 }, fileFilter });
