...
function displayUserProfile(user) {
    ...
    // Populate dropdowns for date of birth if empty
    const ngaySelect = document.getElementById('ngay_sinh');
    const thangSelect = document.getElementById('thang_sinh');
    const namSelect = document.getElementById('nam_sinh');
    
    if (ngaySelect && ngaySelect.options.length <= 1) {
        for (let i = 1; i <= 31; i++) {
            ngaySelect.add(new Option(i, i));
        }
        for (let i = 1; i <= 12; i++) {
            thangSelect.add(new Option(i, i));
        }
        const currentYear = new Date().getFullYear();
        for (let i = currentYear; i >= 1900; i--) {
            namSelect.add(new Option(i, i));
        }
    }

    if (user.ngay_sinh) {
        const date = new Date(user.ngay_sinh);
        if (ngaySelect) ngaySelect.value = date.getDate();
        if (thangSelect) thangSelect.value = date.getMonth() + 1;
        if (namSelect) namSelect.value = date.getFullYear();
    }
    ...
    // Hide change password if user logged in via Google and has no password
    const changePasswordCard = document.getElementById('change-password-card');
    if (changePasswordCard) {
        // has_password might be returned as string "0" or number 0 or false
        if (user.has_password == 0 || user.has_password === false || user.has_password === "0") {
...
async function updateProfile(event) {
    ...
    const ngay = document.getElementById('ngay_sinh')?.value;
    const thang = document.getElementById('thang_sinh')?.value;
    const nam = document.getElementById('nam_sinh')?.value;
    let ngay_sinh = '';
    if (ngay && thang && nam) {
        ngay_sinh = `${nam}-${thang.padStart(2, '0')}-${ngay.padStart(2, '0')}`;
    }

    if (!ten || !ho || !ten_dang_nhap) {
        showToast('Vui lòng nhập tên đăng nhập, tên và họ', 'error');
        return;
    }

    const formData = new FormData();
    formData.append('ten_dang_nhap', ten_dang_nhap);
    formData.append('ho_ten', `${ho} ${ten}`);
    formData.append('so_dien_thoai', sdt);
    if (gioi_tinh) formData.append('gioi_tinh', gioi_tinh);
    if (ngay_sinh) formData.append('ngay_sinh', ngay_sinh);
