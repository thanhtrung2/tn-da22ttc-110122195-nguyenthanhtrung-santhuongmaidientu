    ...
    try {
        ...
        if (userRole === 'seller') {
            ...
            const [result] = await pool.query(
                `INSERT INTO nguoi_dung (ho_ten, email, mat_khau, so_dien_thoai, vai_tro, ten_dang_nhap,
                 cccd_mat_truoc, cccd_mat_sau, giay_phep_kinh_doanh, anh_guong_mat, trang_thai_xac_thuc) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
                [
                    ho_ten, email, hashedPassword, so_dien_thoai || null, userRole, ten_dang_nhap,
        ...
        const [result] = await pool.query(
            'INSERT INTO nguoi_dung (ho_ten, email, mat_khau, so_dien_thoai, vai_tro, ten_dang_nhap) VALUES (?, ?, ?, ?, ?, ?)',
            [ho_ten, email, hashedPassword, so_dien_thoai || null, userRole, ten_dang_nhap]
        );
...
module.exports = { register, login, getProfile, uploadVerification, googleLogin };
